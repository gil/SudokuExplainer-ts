...
- `isRCNotation=false`: chess-style cell names in `Cell.toString` and letter columns in `Grid.Column.toFullString`. Java's other two branches on this flag (`Row.toFullString`, `Column.toFullStringShort`) have identical arms. Only Java's GUI sets it, but it changes recorded hint text, so it gets the same fixture treatment as the solving flags
